...
`/gwh:triage` Step 0.4와 동일 로직. 등록된 모든 계정 enrollment 검증 → `ACCOUNTS` 배열 확정. 0개면 `/gwh:credential-init` 안내 후 중단.

> Unit B Step 0.4 코드 복사 (D7 판단용 카운트: 이 파일이 **3번째 복사** — extract 후보).
