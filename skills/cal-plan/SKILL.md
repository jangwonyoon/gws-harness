...
> **왜 SP→시간 변환이 필요한가**: Jira SP는 상대적 복잡도, 캘린더는 절대 시간. 팀별 속도가 다르므로 기본 테이블(SP1=1.5h)은 팀에 맞게 조정 필요. 사용자가 재조정 기회를 갖도록 명시적으로 출력.
...
> **왜 승인 게이트가 반드시 필요한가**: 한 번 호출로 최대 16개 캘린더 이벤트가 생성될 수 있다. 잘못 배치된 이벤트 수동 삭제 비용(~30분)이 승인 비용(~10초)보다 수백 배 크다. 비대칭적 실수 비용은 승인 게이트로 완화.
